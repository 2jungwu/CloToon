...
- 루트 규칙은 모든 작업에 항상 필요한 공통 규칙만 담습니다.

## Tech Stack

- 앱 프레임워크: Next.js App Router (`app/`), React, TypeScript
- 스타일링: Tailwind CSS v4, `app/styles.css`, shadcn UI, Radix UI, Hugeicons
- 데이터 저장: 로컬 SQLite (`better-sqlite3`), 데이터베이스 파일 `data/app.db`
- API 검증: `zod`
...
- 테스트: 확인 필요. 현재 `package.json`에 `test` 스크립트가 없습니다.
- 브라우저/E2E 테스트: 확인 필요. Playwright는 임시 검증에 사용됐지만 저장소 표준 스크립트는 없습니다.

## Directory Map

- `app/`: Next.js App Router 페이지, 레이아웃, 전역 CSS, API route handler
- `app/api/projects/`: 프로젝트, 컷, 순서 변경, 수정, 삭제를 위한 로컬 JSON API
- `components/`: 공유 React 컴포넌트
- `components/ui/`: shadcn/Radix UI 래퍼 컴포넌트
- `components/projects/`: 프로젝트 목록, 생성, 삭제 UI
- `components/workspace/`: 컷 편집기, HTML 미리보기, mock 이미지, 업로드, PNG/ZIP export
- `lib/db/`: SQLite 연결과 마이그레이션
- `lib/projects/`: 프로젝트 타입과 repository 함수
- `lib/cuts/`: 컷 타입과 repository 함수
- `docs/`: PRD 생성/검증 보조 문서
- `data/`: 로컬 런타임 SQLite 데이터. 사용자 데이터로 취급합니다.
- `artifacts/`: 스크린샷과 다운로드 검증 산출물. 사용자가 요청하지 않으면 커밋하지 않습니다.
...
- import는 `tsconfig.json`과 `components.json`에 맞춰 `@/*` 별칭을 사용합니다.
...
- API 변경 시 `lib/*/types.ts`, repository 구현, route validation을 함께 맞춥니다.
- UI 변경 시 기존 shadcn/Hugeicons 컴포넌트와 `app/styles.css`의 전역 클래스를 우선 사용합니다.
- 삭제 같은 파괴적 작업은 명시적인 사용자 액션을 요구합니다.
- API에서 대상이 없으면 일관되게 `404` 응답을 반환합니다.
- 로컬 데이터 변경 시 `data/app.db`에 사용자의 실제 작업물이 있을 수 있다고 가정합니다.
- 검증용 산출물은 사용자가 요청하지 않으면 커밋하지 않습니다.
...
- 브라우저에 보이는 UI를 바꾸면 `http://127.0.0.1:3000`에서 해당 route를 확인합니다.
...
- 사용자가 provider를 명시적으로 활성화하지 않으면 프로젝트 데이터, 캐릭터 Markdown, 업로드 이미지, API key를 외부 서비스로 보내지 않습니다.
...
- `imagePrompt`: mock 또는 향후 AI 이미지 생성에 쓰는 프롬프트
- `negativePrompt`: 이미지 생성 제외 조건
- `imageDataUrl`: mock 또는 업로드 컷 이미지의 data URL
- `imageStatus`: `"empty"`, `"mock"`, `"uploaded"`, `"generated"`, `"failed"`
- `clo`: PRD에서 언급된 일관 캐릭터 예시 이름

## AI / LLM Engineering Rules

- 최종 텍스트는 HTML/CSS 레이어로 렌더링합니다. 생성 이미지 안의 읽을 수 있는 한국어 텍스트에 의존하지 않습니다.
- 이미지 프롬프트와 자막/대사는 분리합니다.
- 캐릭터 일관성 자산은 사용자 로컬 입력인 캐릭터 Markdown과 표정 이미지로 취급합니다.
- 외부 provider payload는 어떤 데이터가 외부로 나가는지 사용자가 알 수 있어야 합니다.
...
- Gemini provider 모델명, 할당량, 가격, 인증 방식은 저장소에서 확인되지 않았습니다.
