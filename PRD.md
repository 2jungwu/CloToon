...
7. 미리보기에서 자막은 HTML/CSS 레이어로 유지되고, 대사는 생성 이미지 안의 말풍선/대사 표현과 중복 렌더링되지 않는지 확인한다.
...
| F003 | 컷 텍스트 편집 | 컷 시나리오, 자막, 대사, 이미지 프롬프트, 자막 오버레이 스타일을 편집한다. |
...
- `Cut`: `id`, `projectId`, `position`, `template`, `scenario`, `caption`, `dialogue`, `imagePrompt`, `negativePrompt`, `imageDataUrl`, `imageStatus`, `captionStyleOverride`
