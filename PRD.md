...
현재 제품은 local-first 원칙을 따른다. 프로젝트와 컷 데이터는 로컬 SQLite에 저장하고, 캐릭터/배경/폰트/API Key/내보내기 설정은 브라우저 `localStorage`에 저장한다. 외부 이미지 생성은 사용자가 Gemini API Key를 등록하고 워크스페이스에서 `이미지 생성`을 누를 때만 수행한다.

## 2. 현재 구현 기준

- 저장소는 npm workspaces 기반 모노레포다.
- 실제 앱은 `apps/studio`의 Next.js App Router 애플리케이션이다.
- 루트 `npm run dev`, `npm run build`, `npm run lint`, `npm run typecheck`는 `apps/studio` 워크스페이스 스크립트를 실행한다.
- 로컬 서버는 `127.0.0.1:3000`에 바인딩한다.
- 데이터베이스는 루트 `data/app.db`를 사용한다.
- 상단 내비게이션은 `Projects`, `Assets`만 노출한다.
...
- 캐릭터, 배경, 폰트, API Key, 내보내기 설정을 전역 로컬 설정으로 관리한다.
- 컷 텍스트와 이미지 생성 프롬프트를 분리해 관리한다.
- Gemini 이미지 생성 결과를 컷 이미지로 저장하되, 자막과 대사는 이미지 안이 아니라 HTML/CSS 레이어로 유지한다.
...
- 생성 이미지 내부의 읽을 수 있는 한국어 텍스트에 의존하지 않는다.
...
2. `/assets`에서 캐릭터, 배경, 폰트, API Key, 내보내기 설정을 등록한다.
...
7. 미리보기에서 자막과 대사가 HTML/CSS 레이어로 유지되는지 확인한다.
...
| F003 | 컷 텍스트 편집 | 컷 시나리오, 자막, 대사, 이미지 프롬프트, 네거티브 프롬프트를 편집한다. |
| F004 | HTML 컷 미리보기 | 생성/업로드 이미지 위에 자막과 대사를 HTML/CSS 레이어로 렌더링한다. |
| F005 | Assets 통합 설정 | 캐릭터, 배경, 폰트, API Key, 내보내기 설정을 `/assets`에서 관리한다. |
...
- `Cut`: `id`, `projectId`, `position`, `template`, `scenario`, `caption`, `dialogue`, `imagePrompt`, `negativePrompt`, `imageDataUrl`, `imageStatus`

### 8.2 localStorage

- `local-studio-assets`: 캐릭터 세트, 선택 캐릭터, 배경, 폰트.
- `local-studio-settings`: `provider: "gemini"` 호환값, `geminiApiKey`, `exportScale`, `saveOriginalHtml`.
...
- 컷 시나리오, 이미지 프롬프트, 네거티브 프롬프트
- 자막과 대사는 참고 문맥으로만 포함하며 이미지 안에 그리도록 지시하지 않는다.

강제 원칙:

- 이미지 안에 읽을 수 있는 텍스트, 말풍선, 자막, 대사, UI 텍스트, 워터마크를 생성하지 않는다.
- 최종 자막과 대사는 HTML/CSS 레이어로 렌더링한다.
...
- 저장소 표준 `npm test` 스크립트가 없다.
- Playwright는 임시 검증에 사용되지만 표준 E2E 스크립트가 없다.
- Gemini quota/billing 상태는 사용자 Google AI Studio 프로젝트 설정에 의존한다.
- `README.md`, `DESIGN.md`도 현재 구현과 맞는지 별도 검토 후 정리할 필요가 있다.
