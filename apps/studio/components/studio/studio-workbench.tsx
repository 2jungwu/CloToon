...
type CaptionLayerCSSProperties = CSSProperties & Record<`--${string}`, string | number>;
...
function getCaptionLayerStyle(captionStyleInput: CaptionStyle): CaptionLayerCSSProperties {
...
function CaptionLayerEditor({
  onFlushSelectedCut,
  onUpdateSelectedCut,
  selectedCut,
}: CaptionLayerEditorProps) {
  const captionStyle = normalizeCaptionStyle(selectedCut.captionStyle);
    ...
    <section className="caption-layer-editor" aria-label={labels.captionLayerEdit}>
      <div className="caption-editor-section">
        ...
        <div className="caption-editor-toolbar text-toolbar" aria-label={labels.captionTextStyle}>
          ...
          <Select
            onValueChange={(value) =>
              updateCaptionStyle({ text: { fontSize: Number.parseInt(value, 10) } })
            }
          ...
          <button
            ...
            data-active={isBold}
            onClick={() =>
              updateCaptionStyle({ text: { fontWeight: isBold ? 400 : 700 } })
            }
            type="button"
          >
            B
      ...
      <div className="caption-editor-section">
        ...
        <div className="caption-box-toolbar" aria-label={labels.captionBoxStyle}>
          ...
          <CaptionControlField label={labels.borderWidth}>
            <Select
              onValueChange={(value) =>
                updateCaptionStyle({ box: { borderWidth: Number.parseInt(value, 10) } })
              }
          ...
          <CaptionControlField label={labels.borderRadius}>
            <Select
              onValueChange={(value) =>
                updateCaptionStyle({ box: { borderRadius: Number.parseInt(value, 10) } })
              }
...
function CaptionColorControl({
  ...
  function commitColor() {
    const normalized = normalizeHexColor(draftHex) ?? value;
    onChange(normalized);
    setIsOpen(false);
  }
      ...
      {isOpen ? (
        <div className="caption-color-popover" role="dialog" aria-label={label}>
          <div
            ...
            aria-valuetext={`채도 ${Math.round(draftHsv.saturation)}%, 밝기 ${Math.round(
              draftHsv.value,
            )}%`}
          ...
          <div className="caption-color-presets" aria-label={`${label} 프리셋`}>
